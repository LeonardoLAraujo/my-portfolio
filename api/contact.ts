...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    try {
        ...
        if (!name || !email || !message) return res.status(400).json({ success: false, message: "Algum campo está vazio" });

        await db.collection("contatos").add({ name, email, message, criadoEm: new Date() });
        return res.status(200).json({ success: true, message: "Mensagem enviada com sucesso!" });
    } catch (error: any) {
        console.error("Erro no envio do contato:", error.message || error);
